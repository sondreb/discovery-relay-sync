...
export class RelaySync {
  ...
  async publishToTargetRelays(event) {
    ...
    const storageDir = this.config.path;
    ...
    if (storageDir) {
      try {
        ...
        const filePath = path.join(storageDir, `${event.kind}-${event.id}.json`);
        fs.writeFileSync(filePath, JSON.stringify(event, null, 2));

        this.logger.debug(`Saved event ${event.id} to ${filePath}`);
      } catch (error) {
        this.logger.error(`Failed to save event ${event.id} to local storage:`, error);
        // Continue with publishing even if saving fails
      }
    }
